...
public class APIAdminAgentPayPassageController {
    ...
    @GetMapping
    public List<AgentPayPassageDTO> list(
            @RequestParam(required = false,defaultValue = "1") Integer current,
            @RequestParam(required = false, defaultValue = "10") Integer size , AgentPayPassageDTO inputDTO){
        return passageService.selectPage(new Page<>(current,size),inputDTO);
    }
